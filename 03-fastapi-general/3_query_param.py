...
# Path parameter 에 없는 파라미터를 query parameter 한다.
...
# Query parameter 을 required 하게 만들려면? -> 아무값도 넣지 않으면 된다.
# - Optional 을 뺀다.
...
# bool 로 선언해 놓으면 아래 query parameter 가 알아서 'True' 로 타입 컨버전 된다.
...
# <query parameter and path parameter 는 뭐가 다른거야?>
...
#   라고 1가지 내용의 요청만 할 수 있지만, query parameter 에서는 '조건' 을 줄 수 있다는 점이 가장 큰 차이점이다.
# - 예를들어 stock id 가 '32455' 이고 2023-01 월부터 2023-06 월 까지의 가격을 불러와 줘! 라고 한다면 이는 path param 으로 요청하기가 어렵다.
# - 하지만 query parameter 으로는 가능하다. http://www.naver.com/{stock_id}/?start_date=202301&end_date=202306 의 query parameter
#   형태의 API 를 만들면 된다.
#
# 자주 사용되는 곳
# - 필터 / 서치 / 정렬 / Pagination
